...
recent and more diverse opponent populations these are: be nice, be provocable and generous,
...
Harper2017, Kendall2007, Stephens2002, Stewart2012} and today hundreds of more strategies exist
in the literature.
...
Forgiveness in R. Axelrod's terms is a two round procedure. Though essentially
it is the strategy's ability to go from a \(DC\) to \(C\) aiming to achieve
mutual cooperation again, the only way TFT would end up in \(DC\), in
environments without noise, is if it had received a defection and it retaliated.
Following, an opponent's defection if the opponent were to ``apologise''
(resulting in a \(DC\) round), then TFT would forgive its opponent since mutual
cooperation is better than mutual defection.
...
\begin{itemize}
    ...
    \item Reciprocate both cooperation and defection correctly; Be provocable and generous
...
\begin{itemize}
    ...
    \item In noisy environments where a noise probability strictly less than
...
This leads to revisit the recommendation of being provocable to defections made
by R. Axerlod. In fact only in tournament that are not long a strategy should
be too provocable, in the rest of the settings a strategy should be more
generous.
...
computer tournaments presented here is the largest and more diverse collection in the
literature. The \numberofstrategies strategies are drawn from known named
strategies in IPD literature and the computer tournaments include tournaments of
four different settings.

So what is the best way of playing the IPD? And is
there a single dominant strategy for the IPD? There was not a single strategy
...
and demonstrated that there are properties associated with the success of
strategies, contradicting the originally suggested properties of R.
...
the impact of noise such as in the strategy DBS. Moreover, we found some
...
more specifically, to the mean cooperator. Moreover, the manner in which a
strategy achieves a given cooperation rate relative to the tournament population
average is important. In standard tournaments a strategy would aim to be the
mean the cooperator while in noisy tournaments the best performing players
cooperate a lower rate than the tournament population on average.
