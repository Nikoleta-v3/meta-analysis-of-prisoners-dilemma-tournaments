...
salient features. The results imply that there is not yet a single
strategy that performs well in every Iterated Prisoner's Dilemma interaction.
...
mainly due to the strategy's lack of generosity and contrition. The strategy immediately
punishes a defection and in a noisy environment it could lead to a
...
Pavlov (aka Win Stay Lose Shift) ~\cite{Nowak1993}, Generous Tit For Tat~\cite{Nowak1992}.
In~\cite{Press2012} a set of envious IPD strategies
were introduced called zero-determinant strategies (ZDs). By
...
Though only select pieces of work have been discussed, there is a broad number
...
Compared to other works, where a few selected strategies are evaluated
...
possible due to an open source libary called Axelrod-Python (APL),
...
The performance of the strategies is evaluated based on the features of
Table~\ref{table:manual_features}. These feature are measures regarding a
strategy's behaviour and the tournaments the strategies competed in are
explored.
...
FSM 16 Noise 05 is set to 1. The others are calculated as explained in Table
...
defection is indicative of lesser overall success.
...
in probabilistic ending tournaments it has already been mentioned that more defecting
...
or exploitable opponent (e.g. Alternator).
...
Table~\ref{table:manual_features} using random forests can be found in
