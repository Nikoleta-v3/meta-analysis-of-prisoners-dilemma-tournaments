...
strategies in thousands of tournaments, present the top performing strategies across multiple
...
these agents becomes paramount.
\end{abstract}
...
of opponents or representative classes of opponents. This lack of comprehensive
testing against formally defined strategies and tournament winners is
justified given the substantial effort required to implement the numerous
published \IPD{} strategies. The accuracy of implementing prior strategies is
often hindered by insufficient descriptions and a dearth of published
...
Furthermore, our tournaments encompass various variations, including standard
...
contrary, our work demonstrates that such a strategy does not exist. The primary
...
strategies and those training strategies. Moreover, conducting a similar
...
processes.
% ToDO: Not sure if this is enough to justify how our work is important for AI.
% It would be nice to have a sentence or two since it fits the special issue
% theme.
...
observe that \TFT{} has a normally distributed normalized rank. In comparison,
...
the case of noisy and probabilistic ending tournaments.
...
and median score. A strategy would not want to cooperate more than both
the mean and median cooperator in such settings. In probabilistic ending
tournaments, the cooperation rate of the winners and it's relative comparison
to the cooperation rates of the tournament have no effect. The only features
that have an effect are the $CD$ to $C$ rate, which is the tendency of a strategy
to forgive and the SSE rate have a positivist effect on the normalised rank.

A multivariate linear regression has been fitted to model the relationship
between the features and the normalised rank. Based on the graphical
representation of the correlation matrices given in Supplementary Information
several of the features are highly correlated and have been removed before
fitting the linear regression model. The features included are given by
Table~\ref{table:linear_regression} alongside their corresponding \(p\) values
in the distinct tournaments and their regression coefficients. The rate $CD$ to
$C$ has a positively statistically significant effect on the normalized rank,
across all tournament types. This suggests that forgiveness tends to lower one's
performance. In the case of probabilistic ending tournaments, the coefficient of
the $CD$ to $C$ rate is the highest, indicating that one is more harshly
``punished'' when showing forgiveness. Similarly, the SEE error rate has a
...
Table~\ref{table:manual_features} using clustering and random forests can be
found in the Supplementary Information. The results uphold the outcomes of the
correlation and multivariate regression. It also evaluates the effects of the
whether or not a strategy is stochastic, makes use of the knowledge of the
utility values, or makes use of match length. These were not evaluated by the
methods above because there are binary variables. The results showed that they
have no significant effect on a strategy's performance.

The results presented here suggest that forgiveness and a strategy's cooperation
rate, particularly in comparison to the tournament averages, are significant
features. The findings imply that the
forgiveness exhibited by strategies may not consistently contribute to favorable
outcomes. Nevertheless, it is essential to exercise caution in extrapolating the
implications of this analysis. For example, the analysis is limited as we only
...
apart from the case of noisy and probabilistically ending tournaments. In the
case of noisy tournaments, there are winners that did not cooperate exactly as
such but are fairly distributed around 1. Interestingly, in the case of
...
strategies can we winner in probabilistic ending tournaments. The distribution
...
\begin{figure}[!htbp]
    \centering
        ...
        tournaments.} An SSE distribution skewed towards 0 indicates a
        extortionate behaviour by the strategy.}
...
cooperate after such a result, capturing forgiveness, the distributions skew
...
outperforms \TFT{} in most settings. Interestingly, in probabilistic ending
tournaments, there is a peak at 0, suggesting that in such tournaments,
strategies should not be too generous in short matches.

A strategy's effort to make amends is captured by the distributions of the
\(DC\) to \(C\) rates. In noisy tournaments, winners are less likely to
cooperate after a defection; however, there are still behaviors that attempt to
make up for their defection. In standard tournaments and probabilistic ending
tournaments, we observe two hubs of behaviors, one at 0 and one at 0.4-0.5,
indicating that some winners are exploiting whether to try to make up, but with
a small probability. This is also reflected in the \(DD\) to \(D\) rates, where
in standard and probabilistic ending tournaments, there are winners that attempt
to recover from a \(DD\) outcome. The remaining results are as expected, skewed
towards 0.
