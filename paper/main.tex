...
recent and more diverse opponent populations. These are: be nice, be provocable and forgiving,  % TODO modify this based on discussion about vocabulary.
...
Kendall2007, Stewart2012, Harper2017} and today more hundreds of strategies exist  % TODO Order these citations
in the literature.

The winner of both of R. Axelrod's tournaments~\cite{Axelrod1980a, Axelrod1980b}
was the simple strategy Tit For Tat (TFT) which cooperates  % TODO In the appendix we referr to TFT as TfT, we need to be consistent throughout.
...
\begin{itemize}
    ...
    \item Reciprocate both cooperation and defection; Be provocable and forgiving  % TODO Modify this following decision on vocabulary
...
are covered in Section~\ref{section:data_collection}. The data set generated  % TODO Modify this when the raw as well as the processed data is archived
...
that randomly cooperates with probability $C_{\text{mean}}$ is unlikely to be as effective as it will
...
This also explains why Tit For N Tats does not fare well for $N > 1$ as it fails
...
such a scenario the noisy environment effectively voids R. Axelrod's rule to be nice,
...
% TODO Both NG and MH seem happy with this paragraph but I find it quite a
% mouthful and not very too the point (took me a few reads). If you're both
% happy with it that's fine with me but can we just be sure.
...
\item be nice, if the environment isn't noisy and the games are long
    \item be provocable and forgiving,
...
to the authors knowledge, and it available at~\cite{data}. Further data mining % TODO Modify this when the raw data is also archived
