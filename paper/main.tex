...
performance of strategies in the game for decades. The results of the literature,
...
salient features.
The outcomes support that a strategy with a theory of mind should adjust its
...
that could recognise each other and collude to increase one members score.
This resulted in three of the strategies ranking in the top spots. The
...
1. However, the most common values for these probabilities in the literature are
...
Up till now, the performances of the \numberofstrategies strategies have been
evaluated for individual tournament types. The distributions of \(r\) for the
tournament types indicate that for standard, probabilistic ending and noisy
tournaments with \(p_n <0.1\) successful strategies do exist. For these settings
the top 15 strategies frequently ranked in the top spots with only a few
exceptions. The behaviour of the winning strategies in probabilistic ending
tournaments varied when \(p_e\) was restricted to 0.1. For larger values of
\(p_e\) the dominant strategies were defecting ones and for smaller values the
dominant strategies were evolved strategies and strategies based on teams.
Similarly the behaviour
of the dominant strategies varied when the probability of noisy was restricted.
In noisy tournaments with \(p_n < 0.1\) the dominant strategies were strategies
specifically trained for noise tournaments, and the performed well in most of the
tournaments they participated. However, in noisy tournaments with \(p_n \in [0, 1]\)
the performances of the top ranked strategies were bimodal. Indicating that with
high values of noisy a successful set of strategies does not exist.
...
specified, playing as any of the Retaliate strategies will have same the result.
...
of 0.119. If the memory size of a strategy is infinite then memory usage is 1.
...
The performances are evaluated based on different methods.
...
The second method to evaluate the features importance in a strategy's success
...
Finally, a multivariate linear regression is fitted to model the relationship of
Table's~\ref{table:manual_features} and the normalised rank. The \(p-\)values of
