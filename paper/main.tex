...
tournaments. A strategy with a theory of mind would aim to be the mean/median
...
In tournaments with noise and a probability of ending an effective strategy
cooperates 60\% of the times the median/mean cooperator did, and in tourmanents
with only noise or even in a setting that the tournament can be any of the
above a strategy should aim to play 67\% as a median/mean cooperator.
\end{abstract}
...
200 strategies, including ZDs, was ran and none of the them
...
as Tit For 2 Tats~\cite{Axelrod1980b}, Slow Tit For Tats~\cite{axelrodproject}, Hard Tit For 2 Tats~\cite{Stewart2012}
...
and $e$~\cite{axelrodproject}. The Retaliate and
...
of behaviour in noisy tournaments, the \(r\) distribution over the noise
probability \(p_n\) for the top 6 strategies of Figure~\ref{fig:noisy_results},
are given in Figure~\ref{fig:effect_of_noise}.

For \(p_n\) strictly lower than 0.5, Grumpy, Tit For 2 Tats and Slow Tit For Two Tat
have a moderate performing, whereas \(e\), Cycle Hunter and Ricky QLearner are
behaving poorly. At \(p_n=0.5\), all the distributions are bimodal because
with a 0.5 probability of noise any strategies corresponds to a random player.
Interestedly, for a \(p_n\) larger than 0.5 all of the 6 strategies become
successful. Note that a value \(p_n=1\) corresponds to a strategy playing the
exact opposite from what they intended. Thus, the successful strategies in
noisy tournaments presented here, are effective when they are not playing as
themselves. If \(p_n\) was strictly less 0.5 then the top ranked strategies
are different, Table~\ref{table:subset_noise_result}. The top spots are mainly
overtaken by the Meta strategies which are based on teams.
...
\begin{table}[!htbp]
    ...
    \caption{Top performances in noisy tournaments where \(p_n<0.5\).}\label{table:subset_noise_result}
\end{table}
...
it was concluded that they are actually the same strategy. This was not known to
the authors at the time of data collection. Figure~\ref{fig:probend_results}
verifies that their perfomance is the same. Both strategies have repeatedly
...
The distributions of \(r\) over \(p_e\) for the top 6 strategies are also given
by Figure~\ref{fig:effect_of_probend}. It is shown that the rank of the
strategies is decreasing as the probability of the game ending increases and at
the point of \(p_e=0.1\) the strategies become dominant. All the top ranked
strategies in this setting are defecting strategies, and what is exhibited is
that they do better as the probability of the game ending in the next turn
increases, which is inline with the Folk Theorem~\cite{Fudenberg2009}. Consider
tournaments where the probability of the game ending was less than 0.1, the top
ranked strategies based on \(\bar{r}\) are given
by~Table~\ref{table:subset_probend_result}. The top ranked strategies include
strategies based on teams, trained strategies, Grudger and Spiteful Tit for Tat.
...
\begin{table}[!htbp]
    ...
    \caption{Top performances in probabilistic ending tournaments with \(p_e < 0.1\)}
...
$\phi$ ($\pi$, $\phi$ are based on the same approach as $e$) and the Retaliate
family are robust in noisy environments regardless of the
number of turns. However, strategies from the top ranks in probabilistic
ending tournaments did not rank highly here. The distributions of
\(r\) shown in Figure~\ref{fig:noisy_probend_results} have the largest
median values compared to the top rank strategies of the other tournament
types.
...
The distributions of \(r\) for the tournament types indicate that
...
and DoubleCrosser. Figure~\ref{fig:overall_results} gives the normalised rank distributions of
...
Grudger. All the top 15 strategies are fundamentally different. Some are cheaters,
...
top 10\% of the tournament most of the times. Strategies in noisy
environments demonstrated that no strategy can be consistently
successful. Overall, there has been not a single strategy that has shown to perform well in
...
The performances are clustered in two clusters based on the nornalised rank.
More specifically, there are clustered based on whether the normalised rank was
in the top 5\%, the 25\% and the 50\%. A random forest
approach~\cite{breiman2001} is applied to the data to predict the cluster to
which a strategy's performance has been assigned to. The random forest method
...
as well as the accuracy for random forest models, are given in
...
classification tasks; to which cluster a performance was assigned to, have been
...
and several of the measures that are highted by the importance are inline with the
...
Figure~\ref{fig:mean_median_std} the distributions of \(C_ / C_{\text{mean}}\)
and \(C_ / C_{\text{median}}\) are given for the winners. A value of \(C_ /
C_{\text{mean}} = 1\) imply that the cooperating ratio of the winner was the
same as the mean / median cooperating ratio. In standard tournaments, the mean
...
Figure \ref{fig:mean_median_noisy} shows the distributions of the measure for
the winners in noisy tournaments. The mean is at 0.67, thereupon the winners
...
Further data mining could be applied to the data set of this work, available
at~\cite{data}.
