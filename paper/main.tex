...
strategies. Many tournaments have followed Axelrod's~\cite{Bendor1991, Stephens2002,
Kendall2007, Stewart2012, Harper2017} and today more hundreds of strategies exist
in the literature.

Axelrod performed two widely-known computer tournaments~\cite{Axelrod1980a,
Axelrod1980b} in the 1980's. The
winner of both tournaments was the simple strategy Tit For Tat (TFT) which cooperated
...
Other works have built upon the limitations of the TFT, and others have shown
that suggestions made by Axelrod did not necessarily lift to alternative environmental rules.
...
never receive less than their memory-one opponents. While ZD strategies
...
and a pre-selected set of known strategies, outperformed a collection of 12 ZD strategies.
...
allows them to know the number of turns in a match (if they are specified).
...
impact of the prisoner's dilemma being iterated is subdued. This is captured by the Folk
...
Table~\ref{table:manual_features}. These feature are measures regarding a
...
Moreover, the manner in which as strategy achieves a given cooperation rate
relative to the tournament population average is important. Playing a strategy
that randomly cooperates with $C_mean$ is unlikely to be as effective as it will
randomly have cooperations matched with defections. In contrast, TFT naturally
achieves a cooperation rate near $C_mean$ by virtue of copying its opponent's
...
Similarly, these results suggest an explanation regarding the perhaps intuitively unexpected
effectiveness of memory one strategies historically. Given that among the important
features associated to success are the relative cooperation rate to the population
...
exploit weaker strategies. This is why ZD strategies fail to appear in the top
...
\footnote{Note that ZD strategies also tend to perform poorly in population games for
a similar reason: they attempt to exploit other players using ZD strategies, failing to
...
attempted by ZD strategies is not typically effective in tournaments, more
...
These environments mitigate the value of being nice. The
added uncertainty reduces the ability of maintaining mutual cooperation and
also triggers grudging strategies. Accordingly we find that in noisy tournaments
that the best performing players cooperate a lower rate than the tournament
